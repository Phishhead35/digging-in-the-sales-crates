...
export function Deals() {
  ...
  const apiSources = [
    ...
    { name: 'eBay', url: 'https://ebay.com/b/Vinyl-Records/306/bn_1852757', desc: 'Auctions and fixed-price listings. Best for sealed copies, graded records, and quick finds.', tag: 'Live API', color: '#f59e0b' },
    { name: 'CDandLP', url: 'https://www.cdandlp.com', desc: 'European-heavy used marketplace with millions of vinyl listings. Great for international pressings and pricing.', tag: 'Live API', color: '#f59e0b' },
  ];

  // Curated store deep-links (no API)
  const curatedStores = [
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
          {quickSearches.map(({ label, q }) => (
            <button key={q} onClick={() => navigate(`/search?q=${encodeURIComponent(q)}`)} style={{
              ...
              onMouseOver={e => { e.currentTarget.style.borderColor = 'var(--amber)'; e.currentTarget.style.color = 'var(--amber)'; e.currentTarget.style.background = 'var(--amber-glow)'; }}
              onMouseOut={e => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.color = 'var(--text-secondary)'; e.currentTarget.style.background = 'var(--bg-card)'; }}
            >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16, marginBottom: 48 }}>
        {apiSources.map(({ name, url, desc, tag, color }) => (
          <a key={name} href={url} target="_blank" rel="noopener noreferrer" style={{
            ...
            border: '1px solid rgba(245,158,11,0.25)', display: 'block', transition: 'all 0.3s',
          }}
            onMouseOver={e => { e.currentTarget.style.borderColor = 'var(--amber)'; e.currentTarget.style.transform = 'translateY(-3px)'; }}
            onMouseOut={e => { e.currentTarget.style.borderColor = 'rgba(245,158,11,0.25)'; e.currentTarget.style.transform = 'translateY(0)'; }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
        {curatedStores.map(({ name, url, desc, tag, email }) => (
          <a key={name} href={url} target="_blank" rel="noopener noreferrer" style={{
            padding: 24, borderRadius: 16, background: 'var(--bg-card)',
            border: '1px solid var(--border)', display: 'block', transition: 'all 0.3s',
          }}
            onMouseOver={e => { e.currentTarget.style.borderColor = 'var(--border-hover)'; e.currentTarget.style.transform = 'translateY(-3px)'; }}
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              ...
              <span style={{
                ...
                color: '#2ec4b6', fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap',
              }}>{tag}</span>
            </div>
            <p style={{ color: 'var(--text-secondary)', fontSize: 13, lineHeight: 1.6 }}>{desc}</p>
